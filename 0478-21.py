...
#   Let the user decide to continue or not
NewOrder = input("Press ENTER to place an order")
while NewOrder == "":

#   Phone or Tablet Order
    DeviceOrder = input("Would you like to order a phone or tablet Y/N: \n").upper()  

    if DeviceOrder == "Y":
        print("Choose an option 1 - 10")
        for i in range (0,10): 
            print(i+1,PTCodes[i],PTDescriptions[i],PTPrices[i]) #Output phones & chargers

        #   Choose a device & validation 
        PTChoice = int(input ("Enter your choice 1 - 10: "))
        while PTChoice < 1 or PTChoice > 10: #Valdation (Checks that the input is within constraints, if not asks for input again)
            PTChoice = int(input ("Enter your choice 1 - 10: "))

        #   Get the price of the device
        DPrice = PTPrices[PTChoice-1]
